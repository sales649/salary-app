...
st.markdown(f"""
    ...
    <style>
        ...
        /* إطار وتنسيق عالي الوضوح لمربعات رفع الملفات للوضعين */
...
# 🛠️ دالة خيار (أ) المطورة لاستخراج سطر الإجمالي تلقائياً مع تنظيف المبالغ
...
if not st.session_state.get('app_started', False):
    ...
    # 🏛️ موديول ضريبة القيمة المضافة (الخيار أ - المساعد الذكي واستخراج الإجمالي بضغطة زر)
        ...
        st.write('قم برفع شيتات ERP الوعلان المخصصة للفروع وسيقرأ الموديول الإجمالي النهائي تلقائياً مع كامل المرونة لتأكيده أو تعديله:')
        ...
        with v_c1:
            ...
            in_ry_s_net = st.number_input("صافي مبيعات الرياض (قبل الضريبة):", min_value=0.0, value=float(auto_ry_s_net if auto_ry_s_net > 0 else 4282294.51), key="in_ry_s_net")
            in_ry_s_vat = st.number_input("ضريبة مبيعات الرياض (15%):", min_value=0.0, value=float(auto_ry_s_vat if auto_ry_s_vat > 0 else 87824.56), key="in_ry_s_vat")
            ...
            in_ry_r_net = st.number_input("صافي مرتجعات الرياض (قبل الضريبة):", min_value=0.0, value=float(auto_ry_r_net if auto_ry_r_net > 0 else 496719.85), key="in_ry_r_net")
            in_ry_r_vat = st.number_input("ضريبة مرتجعات الرياض (15%):", min_value=0.0, value=float(auto_ry_r_vat if auto_ry_r_vat > 0 else 1618.28), key="in_ry_r_vat")
        ...
        with v_c2:
            ...
            in_jd_s_net = st.number_input("صافي مبيعات جدة (قبل الضريبة):", min_value=0.0, value=float(auto_jd_s_net if auto_jd_s_net > 0 else 1094933.95), key="in_jd_s_net")
            in_jd_s_vat = st.number_input("ضريبة مبيعات جدة (15%):", min_value=0.0, value=float(auto_jd_s_vat if auto_jd_s_vat > 0 else 19849.20), key="in_jd_s_vat")
            ...
            in_jd_r_net = st.number_input("صافي مرتجعات جدة (قبل الضريبة):", min_value=0.0, value=float(auto_jd_r_net if auto_jd_r_net > 0 else 53450.00), key="in_jd_r_net")
            in_jd_r_vat = st.number_input("ضريبة مرتجعات جدة (15%):", min_value=0.0, value=float(auto_jd_r_vat if auto_jd_r_vat > 0 else 1716.15), key="in_jd_r_vat")

        with v_c3:
            st.markdown("#### 📦 المشتريات والمدخلات العامة:")
            ...
            in_p_s_net = st.number_input("صافي المشتريات الخاضعة (15%):", min_value=0.0, value=float(auto_p_s_net if auto_p_s_net > 0 else 953263.81), key="in_p_s_net")
            in_p_s_vat = st.number_input("ضريبة المشتريات (15%):", min_value=0.0, value=float(auto_p_s_vat if auto_p_s_vat > 0 else 138122.66), key="in_p_s_vat")

            st.write("")
            file_purch_ret = st.file_uploader("شيت مرتجعات المشتريات (اختياري):", type=['xlsx', 'xls', 'csv'], key="vat_purch_ret")
            auto_p_r_net, auto_p_r_vat, _ = parse_vat_total_row_smart(file_purch_ret)

            in_p_r_net = st.number_input("صافي مرتجعات المشتريات:", min_value=0.0, value=float(auto_p_r_net), key="in_p_r_net")
            in_p_r_vat = st.number_input("ضريبة مرتجعات المشتريات:", min_value=0.0, value=float(auto_p_r_vat), key="in_p_r_vat")

        # تجميع الإقرار الفوري والمعمد
        total_sales_net = in_ry_s_net + in_jd_s_net
        total_sales_vat = in_ry_s_vat + in_jd_s_vat

        total_sales_ret_net = in_ry_r_net + in_jd_r_net
        total_sales_ret_vat = in_ry_r_vat + in_jd_r_vat

        total_purch_net = in_p_s_net
        total_purch_vat = in_p_s_vat

        total_purch_ret_net = in_p_r_net
        total_purch_ret_vat = in_p_r_vat
        ...
        with btn_v1:
            vat_export_df = pd.DataFrame([
                {'البند': 'مبيعات الرياض الخاضعة للنسبة الأساسية', 'المبلغ قبل الضريبة': in_ry_s_net, 'الضريبة (15%)': in_ry_s_vat},
                {'البند': 'مرتجعات مبيعات الرياض', 'المبلغ قبل الضريبة': in_ry_r_net, 'الضريبة (15%)': in_ry_r_vat},
                {'البند': 'مبيعات جدة الخاضعة للنسبة الأساسية', 'المبلغ قبل الضريبة': in_jd_s_net, 'الضريبة (15%)': in_jd_s_vat},
                {'البند': 'مرتجعات مبيعات جدة', 'المبلغ قبل الضريبة': in_jd_r_net, 'الضريبة (15%)': in_jd_r_vat},
                {'البند': 'المشتريات العامة الخاضعة للنسبة الأساسية', 'المبلغ قبل الضريبة': in_p_s_net, 'الضريبة (15%)': in_p_s_vat},
                {'البند': 'مرتجعات المشتريات', 'المبلغ قبل الضريبة': in_p_r_net, 'الضريبة (15%)': in_p_r_vat},
                {'البند': 'صافي الضريبة المستحقة للهيئة (ZATCA)', 'المبلغ قبل الضريبة': (total_sales_net - total_sales_ret_net) - (total_purch_net - total_purch_ret_net), 'الضريبة (15%)': net_vat_payable}
            ])
